...
class dog{
	...
	/*
	cubeshapeObject legFLS;
	cubeshapeObject legFRS;
	cubeshapeObject legBLS;
	cubeshapeObject legBRS;
	*/
	...
	hingeConstraint hinge_legFLT_legFLS;
	hingeConstraint hinge_legFRT_legFRS;
	hingeConstraint hinge_legBLT_legBLS;
	hingeConstraint hinge_legBRT_legBRS;
	*/
		...
		if(initialDNA == true){
			for(int col = 0; col < dnacol; col++){
				for(int row = 0; row < dnarow; row++){
					dna[row][col] = uniform(-PI/2, PI/2, rnd);
				}
			}
		}
		*/
	...
	void spawn(float x, float y, float z){
		...
		legFLT	= cubeshape_create(createVec3(x+0.5, y-0.75,   z-0.4),
				createVec3(0.2, 0.5, 0.2),		createQuat(1, 0, 0, 0), 0.3);
		legFRT	= cubeshape_create(createVec3(x+0.5, y-0.75,   z+0.4),
				createVec3(0.2, 0.5, 0.2),		createQuat(1, 0, 0, 0), 0.3);
		legBLT		= cubeshape_create(createVec3(x-0.5, y-0.75,   z-0.4),
				createVec3(0.2, 0.5, 0.2),		createQuat(1, 0, 0, 0), 0.3);
		legBRT	= cubeshape_create(createVec3(x-0.5, y-0.75,   z+0.4),
				createVec3(0.2, 0.5, 0.2),		createQuat(1, 0, 0, 0), 0.3);

		/*
		legFLS	= cubeshape_create(createVec3(x+0.5, y-1.25, z-0.4),
				createVec3(0.2, 0.5, 0.2),	createQuat(1, 0, 0, 0), 0.25);
		legFRS	= cubeshape_create(createVec3(x+0.5, y-1.25, z+0.4),
				createVec3(0.2, 0.5, 0.2),	createQuat(1, 0, 0, 0), 0.25);
		legBLS	= cubeshape_create(createVec3(x-0.5, y-1.25, z-0.4),
				createVec3(0.2, 0.5, 0.2),	createQuat(1, 0, 0, 0), 0.25);
		legBRS	= cubeshape_create(createVec3(x-0.5, y-1.25, z+0.4),
				createVec3(0.2, 0.5, 0.2),	createQuat(1, 0, 0, 0), 0.25);
				*/
		...
		hinge_legFLT_legFLS.setLimit(0.0, 1.50);
		hinge_legBLT_legBLS.setLimit(0.0, 1.50);
		hinge_legFRT_legFRS.setLimit(0.0, 1.50);
		hinge_legBRT_legBRS.setLimit(0.0, 1.50);
		...
		hinge_legFLT_legFLS.enableMotor(true);
		hinge_legFLT_legFLS.setMaxMotorImpulse(2);
		hinge_legFRT_legFRS.enableMotor(true);
		hinge_legFRT_legFRS.setMaxMotorImpulse(2);

		hinge_legBLT_legBLS.enableMotor(true);
		hinge_legBLT_legBLS.setMaxMotorImpulse(2);
		hinge_legBRT_legBRS.enableMotor(true);
		hinge_legBRT_legBRS.setMaxMotorImpulse(2);
	...
	void move(int sequence){
		...
		hinge_legFLT_legFLS.setMotorTarget(dna[sequence][4], 0.3);
		hinge_legFRT_legFRS.setMotorTarget(dna[sequence][5], 0.3);
		hinge_legBLT_legBLS.setMotorTarget(dna[sequence][6], 0.3);
		hinge_legBRT_legBRS.setMotorTarget(dna[sequence][7], 0.3);
		*/
	}
	...
	void despawn(){
		...
		hinge_legFLT_legFLS.destroy();
		hinge_legFRT_legFRS.destroy();
		hinge_legBLT_legBLS.destroy();
		hinge_legBRT_legBRS.destroy();
...
extern (C) void tick(){
	...
	if(false){//time == 30 + generation*2){
		...
		//優秀なDNAをコピー
		float[dnarow][dnacol] firstDNA = doglist[$-1].dna;
		float[dnarow][dnacol] secondDNA = doglist[$-2].dna;
		...
		foreach(i; 2..100){
			...
			for(int j = 0; j < numOfAttack; j++){
				doglist[i].dna[uniform(0, dnarow, rnd)][uniform(0, dnacol, rnd)] = uniform(-PI/2, PI/2, rnd);
			}
